"""
Configuration settings for Synvora RAG system
"""
import os
...
CLOUD_LLM_CONFIG = {
    ...
    "max_tokens": int(os.getenv("SYNVORA_CLOUD_MAX_TOKENS", "1024")),
    "temperature": float(os.getenv("SYNVORA_CLOUD_TEMPERATURE", "0.7")),
    "timeout": int(os.getenv("SYNVORA_CLOUD_TIMEOUT", "60")),
}
...
#     service is explicitly enabled via its own SYNVORA_* variable.
#   online_first: prefer cloud services when credentials exist — NIM
#     embeddings + reranking default on, chat defaults to cloud mode.
# SYNVORA_CD is accepted as an alias (earlier name for the same switch);
# hyphenated values normalize to underscores. Explicit SYNVORA_* service
# flags always win over the strategy default.
STRATEGY = (os.getenv("SYNVORA_STRATEGY") or os.getenv("SYNVORA_CD") or "offline_first")
...
NIM_EMBEDDING_CONFIG = {
    "enabled": os.getenv("SYNVORA_NIM_EMBEDDINGS_ENABLED",
                         "true" if ONLINE_FIRST else "false").lower() in {"1", "true", "yes", "on"},
    "api_url": os.getenv("SYNVORA_NIM_EMBEDDING_API_URL", "https://integrate.api.nvidia.com/v1/embeddings"),
    "api_key": os.getenv("SYNVORA_NIM_API_KEY", ""),
    "model": os.getenv("SYNVORA_NIM_EMBEDDING_MODEL", "nvidia/nemotron-3-embed-1b"),
    "dimension": 2048,
    "collection_name": os.getenv("SYNVORA_NIM_COLLECTION_NAME", "synvora_nim_nemotron_3_embed_1b_v1"),
    "timeout": int(os.getenv("SYNVORA_NIM_EMBEDDING_TIMEOUT", "60")),
}
...
NIM_RERANK_CONFIG = {
    "enabled": os.getenv("SYNVORA_NIM_RERANK_ENABLED",
                         "true" if ONLINE_FIRST else "false").lower() in {"1", "true", "yes", "on"},
    "api_url": os.getenv("SYNVORA_NIM_RERANK_API_URL", "https://ai.api.nvidia.com/v1/retrieval/nvidia/reranking"),
    "api_key": os.getenv("SYNVORA_NIM_API_KEY", ""),
    ...
    "model": os.getenv("SYNVORA_NIM_RERANK_MODEL", "nvidia/rerank-qa-mistral-4b"),
    "timeout": int(os.getenv("SYNVORA_NIM_RERANK_TIMEOUT", "30")),
    "max_passages": int(os.getenv("SYNVORA_NIM_RERANK_MAX_PASSAGES", "20")),
}
...
CLIP_IMAGE_CONFIG = {
    ...
    "dimension": 512,
    "collection_name": os.getenv("SYNVORA_CLIP_COLLECTION_NAME", "synvora_clip_images_v1"),
}
...
CHROMA_CONFIG = {
    ...
    "collection_name": "synvora_with_docs",
...
GRAPHIFY_CONFIG = {
    ...
    "install_hint": (
        ...
        "Requires Python 3.10+. Synvora itself can still run without Graphify."
    ),
}

...
class NimReranker:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        if not self.api_url:
            raise ValueError("SYNVORA_NIM_RERANK_API_URL is required when reranking is enabled")
        if not self.api_key:
            raise ValueError("SYNVORA_NIM_API_KEY is required when reranking is enabled")

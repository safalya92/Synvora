...
test.describe("Synvora migration UI", () => {
  ...
  test("upload a text document and show indexing progress", async ({
    ...
    await fileInput.setInputFiles({
      ...
      mimeType: "text/plain",
      buffer: Buffer.from(
        "Synvora Playwright baseline document about offline secure retrieval and citations.",
      ),
    });
  ...
  test("chat page can submit a query", async ({ page }) => {
    ...
    await expect(page.getByText("Synvora", { exact: true }).first()).toBeVisible(
      { timeout: 120_000 },
    );
  });

...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("synvora-theme");
    const preferDark =
      stored === "dark" ||
  ...
  function toggleTheme() {
    ...
    localStorage.setItem("synvora-theme", next ? "dark" : "light");
  }
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex h-12 items-center justify-between border-b border-border bg-surface px-4 sm:hidden">
          <Link href="/chat" className="flex items-center gap-2">
            ...
            <span className="font-semibold tracking-tight text-sm">Synvora</span>
          </Link>

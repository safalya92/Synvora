#!/usr/bin/env python3
"""
Automated build script for Synvora portable executables
"""
import os
import sys
import subprocess
...
class SynvoraBuilder:
    """Automated builder for Synvora portable executables"""
    ...
    def __init__(self):
        ...
        logger.info("Synvora Builder initialized")
    ...
    def build_executable(self) -> bool:
        ...
        try:
            spec_file = self.spec_dir / 'synvora.spec'
            ...
            exe_name = 'synvora.exe' if platform.system() == 'Windows' else 'synvora'
            exe_path = self.dist_dir / 'synvora' / exe_name
    ...
    def package_for_distribution(self) -> bool:
        ...
        try:
            ...
            exe_dir = self.dist_dir / 'synvora'
            if not exe_dir.exists():
                self.log_step("Executable directory not found", False)
                return False
            
            package_name = f"Synvora-{platform.system()}-{platform.machine()}"
    ...
    def create_usb_deployment(self) -> bool:
        ...
        try:
            ...
            exe_dir = self.dist_dir / 'synvora'
            if exe_dir.exists():
                shutil.copytree(exe_dir, usb_dir / 'Synvora', dirs_exist_ok=True)
            
            # Create autorun file for Windows
            if platform.system() == 'Windows':
                autorun_content = """[autorun]
open=Synvora\\synvora.exe
icon=Synvora\\icon.ico
label=Synvora RAG System
"""
...
pause
"""
                with open(usb_dir / 'Launch_Synvora.bat', 'w') as f:
                    f.write(launcher_content)
            else:
                launcher_content = """#!/bin/bash
cd "$(dirname "$0")/Synvora"
./synvora
"""
                launcher_path = usb_dir / 'Launch_Synvora.sh'
                with open(launcher_path, 'w') as f:
                    f.write(launcher_content)
                launcher_path.chmod(0o755)
            
            # Create README for USB
            readme_content = f"""# Synvora USB Deployment

## Quick Start
1. Run Launch_Synvora.{('bat' if platform.system() == 'Windows' else 'sh')}
...
"""
    ...
    def build(self) -> bool:
        ...
        self.log_step("Starting Synvora build process...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Build Synvora portable executables')

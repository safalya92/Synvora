...
class NvidiaNimEmbeddingProvider:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        if not self.api_url:
            raise ValueError("SYNVORA_NIM_EMBEDDING_API_URL is required when NVIDIA NIM embeddings are enabled")
        if not self.api_key:
            raise ValueError("SYNVORA_NIM_API_KEY is required when NVIDIA NIM embeddings are enabled")
        if not self.model:
            raise ValueError("SYNVORA_NIM_EMBEDDING_MODEL is required when NVIDIA NIM embeddings are enabled")

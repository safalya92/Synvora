...
| Check | Result |
|---|---|
| Import `SynvoraGradioApp` / `create_gradio_interface` | **Success** |
| Config load (`GRADIO_CONFIG`, formats, Chroma) | **Success** |
| Chroma collection `synvora_with_docs` | **Connected** (`total_documents`: 0 at baseline) |
...
Current Gradio UI (ui/gradio_app.py)
    → SynvoraGradioApp orchestration (lazy component init)

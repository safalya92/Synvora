# Synvora - Offline Multimodal RAG System
...
Synvora has **two different graph layers**. They are not interchangeable.
...
- Synvora supports older Python runtimes in some deployments.
- Graphify (`graphifyy`) requires **Python 3.10+**.
- Therefore Graphify is **never** a mandatory import-time dependency.
- Synvora talks to Graphify only through its **installed CLI** (`subprocess`, never `shell=True`).
- If Graphify is missing, Synvora still launches; the UI shows install guidance.

### Install Graphify (separate from Synvora)
...
Optional: point Synvora at a custom binary:
...
Graphify semantic extraction uses the same local OpenAI-compatible endpoint as Synvora:

1. Start LM Studio and serve a chat model (e.g. Qwen) on `http://localhost:1234/v1`.
2. Synvora sets `OPENAI_BASE_URL`, `OPENAI_API_KEY`, and `OPENAI_MODEL` for Graphify child processes.
...
- No Graphify package code is imported into core Synvora modules.
...
| Unsupported Python for Graphify | Use Python 3.10+ for the Graphify tool only; Synvora can keep an older runtime |
...
Synvora uses LM Studio for local LLM hosting, providing better performance and easier model management:
...
- Bind hosts explicitly for trusted LAN; keep `SYNVORA_CORS_ORIGINS` tight (no `*`).
...
| CORS errors in browser | `SYNVORA_CORS_ORIGINS` includes `http://127.0.0.1:3000` |
...
For support and issues: [GitHub Issues](https://github.com/thrishank007/Synvora/issues)
